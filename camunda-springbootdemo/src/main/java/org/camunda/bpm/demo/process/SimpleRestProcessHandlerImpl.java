...
//		procReq.getStartProcessInstanceDto().setVariables(variables);
        ...
        if (StringUtils.isNotBlank(procReq.getProcessDefKey())) {
            ...
            processInstance = camundaRestClient.startProcessByKey(procReq.getProcessDefKey(), JSONBuilderProvider.simplest().toJson(procReq.getStartProcessInstanceDto()));
        } else {
            //processInstance = runtimeService.startProcessInstanceById(pscCommonProcessRequest.getProcessDefId(), variables);
            processInstance = camundaRestClient.startProcessById(procReq.getProcessDefId(), JSONBuilderProvider.simplest().toJson(procReq.getStartProcessInstanceDto()));
        }
        // 创建成功
